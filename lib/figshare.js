...
function isAllowedFigshareUrl(raw) {
  let u;
  try { u = new URL(String(raw)); } catch (e) { return false; }
  if (u.protocol !== 'https:') return false;
  const h = u.hostname.toLowerCase();
  return h === 'figshare.com' || h.endsWith('.figshare.com') || h === 'figsh.com' || h.endsWith('.figsh.com');
}

/** Fetch wrapper with retry/backoff. Retries 429 and 5xx. */
async function rawFetch(url, options = {}, { retries = 4, onRetry } = {}) {
  // SSRF barrier: only HTTPS requests to allow-listed Figshare hosts are issued.
  if (!isAllowedFigshareUrl(url)) { const err = new Error('Blocked non-Figshare request'); err.status = 400; throw err; }
  let attempt = 0;
  for (;;) {
    let res;
    try {
